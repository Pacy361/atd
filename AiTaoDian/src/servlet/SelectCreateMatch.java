...
public class SelectCreateMatch extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse response)
		...
		//response.setHeader("content-type","text/html;charset=UTF-8");
		 ...
		 Connection connection=Dbutil.getConnection();
			try { Statement statement=connection.createStatement();//select * from message where message.title like '%小%'
				String sql="select username,mes_id,phone,title,content,time,message.task from god join message on message.user_id=god.phone " +
						"where title like '%"+nameStr+"%' and  spec=5";//
